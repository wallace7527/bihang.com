...
const NULL_STRING = "_#wangLei@nullstring"
...
func NewModel(table string) *Model{
	return &Model{db:defaultDatabase,tablename:table}
}

func (m *Model) Get() map[int]map[string]string {
...
func (m *Model) Take( n int ) map[int]map[string]string {
...
func (m *Model) Insert(param map[string]interface{}) (num int, err error) {
	...
	for key, value := range param {
		...
		case int32:
			values = append(values, strconv.FormatInt(int64(value.(int32)),10))
		case int64:
		...
		case sql.NullString:
			values = append(values, NULL_STRING)

		}
	}

	fileValue := "'" + strings.Join(values, "','") + "'"
	fileValue = strings.Replace(fileValue, "'"+NULL_STRING+"'", "null", -1)
...
func (m *Model) Update(param map[string]interface{}) (num int, err error) {
	...
	for key, value := range param {
		...
		case int32:
			set := fmt.Sprintf("%v = %v", key, value.(int32))
			setValue = append(setValue, set)
		case int64:
		...
		case float32, float64:
			...
			setValue = append(setValue, set)
		case time.Time:
			set := fmt.Sprintf("%v = '%v'", key, fmt.Sprintf("%s", value.(time.Time).Format("2006-01-02 15:04:05")))
			setValue = append(setValue, set)
		case sql.NullString:
			set := fmt.Sprintf("%v = null", key)
			setValue = append(setValue, set)
		}
...
func (m *Model) Count() int64 {
	sql := fmt.Sprintf("select count(*) as c from %v %v", m.tablename, m.where)
	r := m.Query(sql).(map[int]map[string]string)
	id, _ := strconv.ParseInt(r[1]["c"], 10, 64)
	return id
}
